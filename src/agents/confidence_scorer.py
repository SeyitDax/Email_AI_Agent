...
class ConfidenceScorer:
    ...
    def analyze_confidence(self, classification_result: Dict, email_content: str) -> ConfidenceAnalysis:
        ...
        recommended_action = self._determine_action(overall_confidence, risk_factors, risk_score)
        
        # 6. Check which thresholds were met
        confidence_threshold_met = {
            'send': overall_confidence >= self.thresholds['send'],
            'review': overall_confidence >= self.thresholds['review'],
            'escalate': overall_confidence < self.thresholds['escalate']
        }
    ...
    def _determine_action(self, overall_confidence: float, risk_factors: List[RiskFactor],
                         risk_score: float) -> ProcessingAction:
        """Determine the recommended processing action"""
        ...
        # Standard confidence-based decision
        if overall_confidence >= self.thresholds['send'] and risk_score < 0.3:
            return ProcessingAction.SEND
        elif overall_confidence >= self.thresholds['review']:
    ...
    def _generate_reasoning(self, overall_confidence: float, classification_confidence: float,
                          ...
                          action: ProcessingAction) -> List[str]:
